...
Future<void> showTtlDialog(BuildContext context, WidgetRef ref, {required String database}) {
  return showDialog(
    context: context,
...
class _TtlDialogState extends ConsumerState<TtlDialog> {
  ...
  Future<void> _submit() async {
    if (_mode == _TtlMode.custom) {
      ...
      if (n == null || n < 1) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TTL 需为正整数毫秒')));
        return;
      }
    }
    ...
    try {
      final sql = switch (_mode) {
        _TtlMode.custom => SqlBuilder.setTtl(widget.database, ttlMs: int.parse(_ttl.text.trim())),
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('TTL 已更新')));
          ...
          child: Column(
            ...
            children: [
              const RadioListTile<_TtlMode>(
                ...
                title: Text('自定义 TTL', style: TextStyle(fontSize: TdTokens.fontBody)),
                value: _TtlMode.custom,
              ),
              Padding(
                padding: const EdgeInsets.only(left: 36, bottom: TdTokens.space2),
                child: TextField(
                  controller: _ttl,
              ...
              const RadioListTile<_TtlMode>(
                contentPadding: EdgeInsets.zero,
                title: Text('永久保存（INF）', style: TextStyle(fontSize: TdTokens.fontBody)),
                value: _TtlMode.infinite,
              ),
              const RadioListTile<_TtlMode>(
                contentPadding: EdgeInsets.zero,
                title: Text('取消 TTL（UNSET）', style: TextStyle(fontSize: TdTokens.fontBody)),
